...
return function(shared, page)
    ...
    task.spawn(function()
        while true do
            if #logStack > 0 then
                -- Processa até 10 logs de uma vez (evita travar o jogo se spawnar 500 balas)
                for i = 1, math.min(#logStack, 10) do
                    appendLog()
                end
            end
            -- Espera o próximo tick. Muito mais leve que RenderStepped
            task.wait()
        end
    end)
    ...
    if hooksAvailable then
        ...
        task.spawn(function()
            ...
            -- OTIMIZAÇÃO: Filtra os objetos ANTES de chamar a função pesada
            local function checkAndSetup(v)
            ...
            if getfenv().getinstances then
                for _, v in getfenv().getinstances() do checkAndSetup(v) end
            end
            if getfenv().getnilinstances then
                for _, v in getfenv().getnilinstances() do checkAndSetup(v) end
            end

            for _, v in game:GetDescendants() do checkAndSetup(v) end
            
            -- Agora ele só processa o que for Bindable! Zero lag de criação de peças.
            cons[#cons + 1] = game.DescendantAdded:Connect(checkAndSetup)
        end)
    end
...
return function(shared, page)
    ...
    local decompiled = { }
	local bytecodeCache = { } -- NOVA TABELA PARA EVITAR REPETIÇÃO
 ...
 task.spawn(function()
    local function dec(scr)
        ...
        local ok, bytecode = pcall(function()
            return gsbc and gsbc(scr) or ""
        end)

        if not ok then bytecode = "" end

        if (game:GetService("CoreGui") and not scr:IsDescendantOf(game:GetService("CoreGui")) and not scr:IsDescendantOf(game:GetService("CorePackages")) or not game:GetService("CoreGui")) 
            and (scr:IsA("LocalScript") or scr:IsA("ModuleScript") or scr:IsA("Script") and scr.RunContext == Enum.RunContext.Client) 
            and not decompiled[scr] 
            and #bytecode ~= 0 then

            scripts += 1

            local decompi
            if bytecodeCache[bytecode] then
                decompi = bytecodeCache[bytecode]
            else
                decompi = decompile(scr)
                bytecodeCache[bytecode] = decompi
            end
...
metatable = setmetatable({ }, {
    ...
    __newindex = function(self, index, value)
        ...
        if auto and auto[2] then
            if files then
                -- OTIMIZAÇÃO: Debounce para não travar o disco ao arrastar sliders
                if self.SaveThread then
                    task.cancel(self.SaveThread)
                end
                
                self.SaveThread = task.delay(0.5, function()
                    local saveFile = { }
                    for _, val in self.Values do
                        local i = val[1]
                        local v = val[2]
                    ...
                    pcall(function()
                        getfenv().writefile("Ketamine/Settings.json", http:JSONEncode(saveFile))
                    end)
                end)
            end
        end
...
return function(shared, page)
    ...
    task.spawn(function()
        ...
        -- OTIMIZAÇÃO: Filtra os objetos ANTES de chamar a função pesada
        local function checkAndSetup(v)
            if v:IsA("RemoteEvent") or v:IsA("UnreliableRemoteEvent") or v:IsA("RemoteFunction") then
                task.spawn(setup, v)
            end
        end
        ...
        -- Agora ele só processa o que for Remote! Zero lag de criação de peças.
        cons[#cons + 1] = game.DescendantAdded:Connect(checkAndSetup)
    end)
